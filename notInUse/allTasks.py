...
def artificialNeuralNetwork(windspeed, power, forecastInput, actualPower, showPlots):
...
def runAllModelsTask1(showPlots):
    ...
    ANN = artificialNeuralNetwork(windspeed_training, power_training, windspeed_input, actualPower, showPlots)
...
def runAllModelsTask2(showPlots):
    simpleLinearRegression = linearRegression(windspeed_training, power_training, windspeed_input, actualPower,
                                              showPlots)
...
def runAllModelsTask3(actualPower, windowsize, showPlots):
    x, y = createTimeserie(power_training, windowsize)
    ...
    if showPlots:
        plotResults3Inputs(actualPower, tsLinReg[0], tsSVR[0], "actual power", "linear regression",
                           "supported vector regression",
                           "actual power, linear regression and SVR comparison for timeseries")

        plotResults3Inputs(actualPower, tsANN[0], tsRNN[0], "actual power", "artificial neural network",
...
runAllModelsTask3(actualPower, 1, True)
